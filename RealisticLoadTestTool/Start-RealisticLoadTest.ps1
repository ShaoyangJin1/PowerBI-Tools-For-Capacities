...
# 全局错误捕获：出错时显示错误信息并暂停，不闪退
trap {
    Write-Host "`n[错误] $_" -ForegroundColor Red
    Write-Host "请截图以上错误信息。" -ForegroundColor Yellow
    pause
    exit 1
}
...
[System.IO.File]::WriteAllText((Join-Path $testDir "PBIToken.JSON"), $tokenContent, [System.Text.Encoding]::UTF8)
...
[System.IO.File]::WriteAllText((Join-Path $testDir "PBIReport.JSON"), $reportContent, [System.Text.Encoding]::UTF8)

# RealisticLoadTest.html — 读取主模板，替换 CDN 为本地 JS
...
[System.IO.File]::WriteAllText((Join-Path $testDir $htmlFile), $htmlContent, [System.Text.Encoding]::UTF8)
...
if ([string]::IsNullOrWhiteSpace($chromePath) -or !(Test-Path $chromePath)) {
    throw "未找到 Chrome，请在此机器上安装 Google Chrome 后重试。"
}
